...
const initialTeamMembers = [
  { id: 'usr_shyamsundar', name: 'Shyamsundar Varma', email: 'shyamsundar@urbangaon.com', avatar: 'SV', color: '#f59e0b', role: 'member', pin: '1234', status: 'offline', last_heartbeat: new Date(0) },
...
const inFlightTaskCreations = new Map();

// Enterprise Input Sanitization against XSS and Script Injections
...
// In-Memory Sliding Window Rate Limiter for PIN and API Abuse Prevention
const rateLimitMap = new Map();
function checkRateLimit(key, maxAttempts = 5, windowMs = 60000) {
  const now = Date.now();
  const record = rateLimitMap.get(key) || { count: 0, firstAttempt: now, blockedUntil: 0 };
  ...
  if (record.count > maxAttempts) {
    ...
    return { allowed: false, remainingSec: Math.ceil(windowMs / 1000) };
  }
...
const dbHelpers = {
  ...
  // Auth: Verify 4-digit PIN with Sliding-Window Brute-Force Rate Limiting
  verifyPin: async (userId, pin, clientIp = 'unknown') => {
    const rateLimitKey = `pin_${userId}_${clientIp}`;
    const rateCheck = checkRateLimit(rateLimitKey, 5, 60000);

    if (!rateCheck.allowed) {
      return {
        success: false,
        error: `Security Lockout: Too many failed attempts. Please wait ${rateCheck.remainingSec}s.`,
        rateLimited: true,
  ...
  createTask: async (taskData) => {
    const cleanTitle = sanitizeText(taskData.title || 'Untitled Task');
    const lockKey = `${cleanTitle}|${taskData.assigned_to}`;

    // SDE-3 & CEO In-Flight & Idempotency Lock: Deduplicate concurrent or rapid requests
    if (!taskData.is_book_reading && cleanTitle) {
      if (inFlightTaskCreations.has(lockKey)) {
        return inFlightTaskCreations.get(lockKey);
      }
    }

    const creationPromise = (async () => {
      // SDE-3 & CEO Idempotency Guard: Check in-memory database
      if (!taskData.is_book_reading && cleanTitle) {
        const recentMemDup = memoryDb.tasks.find(t =>
          !t.is_book_reading &&
          t.title === cleanTitle &&
          t.assigned_to === taskData.assigned_to &&
          t.created_at && (Date.now() - new Date(t.created_at).getTime()) < 4000
        );
        if (recentMemDup) {
          return await dbHelpers.getTaskById(recentMemDup.id);
        }
      }
      ...
      const newTask = {
        ...
        remarks: Array.isArray(taskData.remarks) ? taskData.remarks : (taskData.initial_remark ? [{
          id: 'rem_' + Date.now().toString(36) + Math.random().toString(36).substr(2, 3),
          text: taskData.initial_remark.trim(),
          author_id: taskData.created_by || 'usr_shyamsundar',
          author_name: taskData.creator_name || 'Admin',
          author_avatar: (taskData.creator_name || 'AD').substring(0, 2).toUpperCase(),
          author_color: '#6366f1',
          created_at: new Date()
        }] : []),
        latest_remark: taskData.latest_remark || (taskData.initial_remark ? taskData.initial_remark.trim() : '')
      };

      try {
        if (await connectToMongo()) {
          if (!taskData.is_book_reading && cleanTitle) {
            const recentMongoDup = await Task.findOne({
              is_book_reading: false,
              title: cleanTitle,
              assigned_to: taskData.assigned_to,
              createdAt: { $gte: new Date(Date.now() - 4000) }
            }).lean();
            if (recentMongoDup) {
              return await dbHelpers.getTaskById(recentMongoDup.id);
            }
          }
    ...
    if (!taskData.is_book_reading && cleanTitle) {
      inFlightTaskCreations.set(lockKey, creationPromise);
      setTimeout(() => inFlightTaskCreations.delete(lockKey), 4000);
    }
  ...
  addRemark: async (taskId, remarkData, user) => {
    const rawText = typeof remarkData === 'string' ? remarkData : (remarkData?.text || '');
    const text = sanitizeText(rawText);
    if (!text) return null;

    const authorId = user?.id || remarkData?.author_id || 'usr_unknown';
    const authorName = sanitizeText(user?.name || remarkData?.author_name || 'Team Member');
    const authorAvatar = user?.avatar || remarkData?.author_avatar || authorName.substring(0, 2).toUpperCase();
    const authorColor = user?.color || remarkData?.author_color || '#6366f1';

    // Idempotency Guard: prevent duplicate remark creation if identical text & author posted within 3 seconds
    const memTask = memoryDb.tasks.find(t => t.id === taskId);
    if (memTask && Array.isArray(memTask.remarks) && memTask.remarks.length > 0) {
      const latest = memTask.remarks[0];
      const timeDiff = Math.abs(Date.now() - new Date(latest.created_at).getTime());
      if (latest.text === text && latest.author_id === authorId && timeDiff < 3500) {
        return dbHelpers.getTaskById(taskId);
      }
    }
    ...
    // Update in-memory operational cache
    if (memTask) {
      if (!Array.isArray(memTask.remarks)) memTask.remarks = [];
      memTask.remarks.unshift(remarkEntry);
      memTask.latest_remark = text;
    }

    try {
      if (await connectToMongo()) {
        // Mongo Idempotency Check
        const existingTask = await Task.findOne({ id: taskId }).lean();
        if (existingTask && Array.isArray(existingTask.remarks) && existingTask.remarks.length > 0) {
          const latestMongo = existingTask.remarks[0];
          const timeDiff = Math.abs(Date.now() - new Date(latestMongo.created_at).getTime());
          if (latestMongo.text === text && latestMongo.author_id === authorId && timeDiff < 3500) {
            return await dbHelpers.getTaskById(taskId);
          }
        }

        // Pure atomic $push ($position: 0) and $set: no race conditions under high concurrency
        ...
        if (updatedTask) {
          await dbHelpers.logActivity(
            ...
            authorName,
            'task_remark',
            `Remark on "${updatedTask.title}": "${text.length > 55 ? text.substring(0, 55) + '...' : text}"`,
            taskId
          );
      ...
      console.error('addRemark DB error:', e);
    }
    ...
    if (memTask) {
      await dbHelpers.logActivity(
        ...
        authorName,
        'task_remark',
        `Remark on "${memTask.title}": "${text.length > 55 ? text.substring(0, 55) + '...' : text}"`,
        taskId
      );
      return dbHelpers.getTaskById(taskId);
    }
  ...
  deleteRemark: async (taskId, remarkId, user) => {
    if (!taskId || !remarkId) return null;

    let removedRemark = null;

    // Update in-memory operational cache
    const memTask = memoryDb.tasks.find(t => t.id === taskId);
    if (memTask && Array.isArray(memTask.remarks)) {
      const idx = memTask.remarks.findIndex(r => r.id === remarkId || r._id === remarkId || String(r._id) === String(remarkId));
      if (idx !== -1) {
        removedRemark = memTask.remarks[idx];
        memTask.remarks.splice(idx, 1);
        memTask.latest_remark = memTask.remarks.length > 0 ? memTask.remarks[0].text : '';
      }
    }

    try {
      if (await connectToMongo()) {
        // Atomic $pull to remove matching remark
        const pullQuery = mongoose.Types.ObjectId.isValid(remarkId)
          ? { $or: [{ id: remarkId }, { _id: new mongoose.Types.ObjectId(remarkId) }] }
          : { id: remarkId };

        const updatedDoc = await Task.findOneAndUpdate(
          { id: taskId },
          { $pull: { remarks: pullQuery } },
          { returnDocument: 'after' }
        ).lean();

        if (updatedDoc) {
          const latest = updatedDoc.remarks && updatedDoc.remarks.length > 0 ? updatedDoc.remarks[0].text : '';
          await Task.updateOne({ id: taskId }, { $set: { latest_remark: latest } });

          await dbHelpers.logActivity(
            user?.id || 'usr_unknown',
            user?.name || 'Team Member',
            'task_remark_deleted',
            `Deleted remark on "${updatedDoc.title}"`,
            taskId
          );
      ...
      console.error('deleteRemark DB error:', e);
    }
    ...
    if (memTask) {
      await dbHelpers.logActivity(
        ...
        user?.name || 'Team Member',
        'task_remark_deleted',
        `Deleted remark on "${memTask.title}"`,
        taskId
      );
      return dbHelpers.getTaskById(taskId);
    }
  ...
  updateTask: async (id, updates) => {
    ...
    memoryDb.tasks[index] = {
      ...
      remarks: updates.remarks || memoryDb.tasks[index].remarks || [],
      latest_remark: updates.latest_remark !== undefined ? updates.latest_remark : (memoryDb.tasks[index].latest_remark || '')
    };
    return dbHelpers.getTaskById(id);
  },
