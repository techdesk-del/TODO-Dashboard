...
// 9 Official Team Members with default PIN: 1234
const initialTeamMembers = [
  { id: 'usr_shyamsundar', name: 'Shyamsundar Varma', email: 'shyamsundar@company.io', avatar: 'SV', color: '#f59e0b', role: 'ceo', pin: '1234', status: 'offline' },
  { id: 'usr_aakash', name: 'Aakash Das', email: 'aakash.das@company.io', avatar: 'AD', color: '#6366f1', role: 'admin', pin: '1234', status: 'offline' },
  { id: 'usr_yudhister', name: 'Yudhister Tiwari', email: 'yudhister.t@company.io', avatar: 'YT', color: '#10b981', role: 'member', pin: '1234', status: 'offline' },
  { id: 'usr_rekha', name: 'Dr Rekha Pareek', email: 'rekha.pareek@company.io', avatar: 'RP', color: '#a855f7', role: 'member', pin: '1234', status: 'offline' },
  { id: 'usr_sanjay', name: 'Sanjay', email: 'sanjay@company.io', avatar: 'SJ', color: '#06b6d4', role: 'member', pin: '1234', status: 'offline' },
  { id: 'usr_ayaz', name: 'Ayaz', email: 'ayaz@company.io', avatar: 'AY', color: '#ec4899', role: 'member', pin: '1234', status: 'offline' },
  { id: 'usr_utkarsh', name: 'Utkarsh', email: 'utkarsh@company.io', avatar: 'UT', color: '#3b82f6', role: 'member', pin: '1234', status: 'offline' },
  { id: 'usr_pratap', name: 'Pratap', email: 'pratap@company.io', avatar: 'PR', color: '#14b8a6', role: 'member', pin: '1234', status: 'offline' },
  { id: 'usr_varun', name: 'Varun Mudgal', email: 'varun.mudgal@company.io', avatar: 'VM', color: '#f97316', role: 'member', pin: '1234', status: 'offline' }
];
...
async function connectToMongo() {
  ...
  try {
    ...
    // Ensure all 9 members exist and have PIN & role in Atlas
...
const dbHelpers = {
  ...
  // Users (sanitized without pin in response)
  getUsers: async () => {
    try {
      if (await connectToMongo()) {
    ...
    return memoryDb.users.map(({ pin, ...safe }) => safe);
  },
  ...
  syncOnlinePresence: async (activeUserIds) => {
    ...
    try {
      if (await connectToMongo()) {
        await User.updateMany({ id: { $in: activeUserIds } }, { status: 'online', last_active: new Date() });
        await User.updateMany({ id: { $nin: activeUserIds } }, { status: 'offline' });
        return await User.find({}, { pin: 0 }).lean();
      }
    } catch (e) {}

    memoryDb.users.forEach(u => {
      u.status = activeSet.has(u.id) ? 'online' : 'offline';
    });
    return memoryDb.users.map(({ pin, ...safe }) => safe);
  },
  ...
  createTask: async (taskData) => {
    const id = 'tsk_' + Date.now().toString(36) + Math.random().toString(36).substr(2, 4);
    const newTask = {
      id,
  ...
  updateTask: async (id, updates) => {
    try {
      if (await connectToMongo()) {
        ...
        await Task.findOneAndUpdate({ id }, updates, { new: true });
        return await dbHelpers.getTaskById(id);
      }
    } catch (e) {}
    ...
    memoryDb.tasks[index] = { ...memoryDb.tasks[index], ...updates };
    return dbHelpers.getTaskById(id);
  },
  ...
  createEodReport: async (report) => {
    ...
    try {
      if (await connectToMongo()) {
        ...
        await User.findOneAndUpdate({ id: report.user_id }, { status: 'logged_out', last_active: new Date() });
        return newReport;
      }
    } catch (e) {}
