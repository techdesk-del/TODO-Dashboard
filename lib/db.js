...
const dbHelpers = {
  ...
  addRemark: async (taskId, remarkData, user) => {
    ...
    const authorName = user?.name || remarkData?.author_name || 'Team Member';
    ...
    try {
      if (await connectToMongo()) {
        const existingTask = await Task.findOne({ id: taskId }).lean();
        if (existingTask) {
          const currentRemarks = Array.isArray(existingTask.remarks) ? [...existingTask.remarks] : [];
          currentRemarks.unshift(remarkEntry);
          ...
          await dbHelpers.logActivity(
            ...
            authorName,
            'task_remark',
            `Remark on "${existingTask.title}": "${text.length > 55 ? text.substring(0, 55) + '...' : text}"`,
            taskId
          );
  ...
  deleteRemark: async (taskId, remarkId, user) => {
    ...
    try {
      if (await connectToMongo()) {
        ...
        if (existingTask) {
          const currentRemarks = Array.isArray(existingTask.remarks) ? [...existingTask.remarks] : [];
          const idx = currentRemarks.findIndex(r => r.id === remarkId || r._id === remarkId || String(r._id) === String(remarkId));
          if (idx !== -1) {
            removedRemark = removedRemark || currentRemarks[idx];
            currentRemarks.splice(idx, 1);
          }
          const latest = currentRemarks.length > 0 ? currentRemarks[0].text : '';
          ...
          await dbHelpers.logActivity(
            ...
            user?.name || 'Team Member',
            'task_remark_deleted',
            `Deleted remark on "${existingTask.title}"${removedRemark?.text ? `: "${removedRemark.text.length > 40 ? removedRemark.text.substring(0, 40) + '...' : removedRemark.text}"` : ''}`,
            taskId
          );
