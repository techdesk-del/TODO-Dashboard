        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5 max-h-[75vh] overflow-y-auto">
          ...
          {!timeStatus.isAllowed && (
            <div className="p-4 rounded-xl bg-amber-50 border border-amber-300 shadow-xs flex flex-col gap-2.5">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className="text-[10px] px-2.5 py-0.5 rounded-full bg-amber-200 text-amber-900 font-extrabold border border-amber-300 flex items-center gap-1">
                      ...
                      {timeStatus.formattedRemaining} left
                    </span>
                  </div>
              ...
              {/* Executive CEO Bypass Option */}
              {isAakash && (
                <div className="mt-1 pt-2.5 border-t border-amber-200/80 flex items-center justify-between gap-3 bg-white/80 p-2.5 rounded-lg">
                  <div className="flex items-center gap-2">
                    <Crown className="w-4 h-4 text-amber-600 shrink-0" />
                    <span className="text-xs font-bold text-slate-800">
                      Executive Admin Testing Override:
                    </span>
                  </div>
                  <label className="flex items-center gap-2 cursor-pointer select-none">
                    ...
                    />
                    <span className="text-xs font-bold text-indigo-700 hover:text-indigo-900">
                      Authorize Early Checkout (CEO Access)
                    </span>
                  </label>
                </div>
              )}
          ...
          <div className="pt-3 flex flex-col sm:flex-row items-center justify-between gap-3 border-t border-slate-100">
            <div className="text-xs text-slate-500 w-full sm:w-auto">
              {!canSubmit ? (
                ...
                <span className="text-emerald-700 font-semibold flex items-center gap-1.5 bg-emerald-50 px-2.5 py-1 rounded-lg border border-emerald-200/80">
                  ...
                  {isAakash && ceoOverride && !timeStatus.isAllowed ? 'CEO Override Active • Ready' : 'Shift Period Complete • Ready'}
                </span>
              )}
            </div>
            ...
            <div className="flex items-center gap-2.5 w-full sm:w-auto justify-end">
              <button
                type="button"
                onClick={onClose}
                className="px-4 py-2 text-xs font-semibold text-slate-600 hover:bg-slate-100 rounded-lg cursor-pointer transition-all"
              >
                Cancel
              </button>
              <button
