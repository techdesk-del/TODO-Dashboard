...
export default function TaskCard({
  ...
  const priorityStyles = {
    urgent: 'border-l-rose-500 hover:border-l-rose-600',
    high: 'border-l-amber-500 hover:border-l-amber-600',
    medium: 'border-l-indigo-500 hover:border-l-indigo-600',
    low: 'border-l-slate-300 hover:border-l-slate-400'
  };
  ...
  const priorityBadges = {
    ...
    medium: (
      <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-700 border border-indigo-200/80">
        <span className="w-1.5 h-1.5 rounded-full bg-indigo-500" />
        Medium
      </span>
    ),
    low: (
      <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full bg-slate-50 text-slate-600 border border-slate-200/80">
  ...
  return (
    <div
      ...
      className={`group relative rounded-2xl bg-white p-3.5 transition-all duration-200 border border-slate-200/80 border-l-[3.5px] flex flex-col justify-between shadow-[0_1px_3px_0_rgba(15,23,42,0.03)] hover:shadow-[0_8px_20px_-4px_rgba(15,23,42,0.08)] hover:-translate-y-0.5 cursor-grab active:cursor-grabbing select-none ${priorityStyles[task.priority] || 'border-l-indigo-500'
        } ${isCompleted ? 'opacity-70 bg-slate-50/60' : 'bg-white'} ${deadlineInfo?.type === 'overdue' ? 'ring-1 ring-rose-400 shadow-rose-100/50' : ''
        }`}
    >
      {/* Top Header Row */}
      <div className="flex items-center justify-between gap-1.5 h-6 mb-2">
        <div className="flex items-center gap-1.5 flex-wrap min-w-0">
          ...
          {deadlineInfo && (
            <span className={`text-[10px] px-2 py-0.5 rounded-full inline-flex items-center gap-1 h-5 ${deadlineInfo.className}`}>
              {deadlineInfo.icon}
          ...
          {isBlocked && (
            <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-rose-50 text-rose-700 border border-rose-200 inline-flex items-center gap-1 h-5">
        ...
        <div className="flex items-center gap-0.5 shrink-0 opacity-80 group-hover:opacity-100 transition-opacity">
          <button
            ...
            className={`h-6 px-1.5 rounded-lg flex items-center gap-1 transition-all cursor-pointer ${task.remarks?.length > 0
                ? 'bg-indigo-50 text-indigo-700 font-bold text-[10px] border border-indigo-200/80 shadow-2xs'
          ...
          <button
            ...
            title="Edit Task"
            className="w-6 h-6 rounded-lg flex items-center justify-center text-slate-400 hover:text-indigo-600 hover:bg-slate-100 transition-all cursor-pointer"
          >
          ...
          <button
            ...
            title="Delete Task"
            className="w-6 h-6 rounded-lg flex items-center justify-center text-slate-400 hover:text-rose-600 hover:bg-rose-50 transition-all cursor-pointer"
          >
      ...
      <div className="flex items-start gap-2 pt-0.5">
        <button
          ...
          className={`mt-0.5 w-4 h-4 rounded-md border flex items-center justify-center transition-all shrink-0 cursor-pointer ${isCompleted
        ...
        <div className="flex-1 min-w-0">
          {task.is_book_reading && (
            <span className="text-[9.5px] font-bold uppercase tracking-wide text-indigo-600 mb-0.5 flex items-center gap-1">
              <BookOpen className="w-3 h-3 text-indigo-600" /> Book Reading Library
            </span>
          )}
          <h4 className={`text-[12.5px] font-semibold tracking-tight leading-snug break-words ${isCompleted ? 'text-slate-400 line-through' : 'text-slate-900 group-hover:text-indigo-950 transition-colors'
            }`}>
            {task.title}
          </h4>
          {task.description && !task.is_book_reading && (
            <p className="text-[11px] mt-1 line-clamp-2 leading-relaxed text-slate-500 font-normal break-words">
              {task.description}
            </p>
          )}
          ...
          {(task.latest_remark || task.remarks?.[0]?.text) && (
            <div
              ...
              className="mt-2 p-2 rounded-xl bg-slate-50 hover:bg-indigo-50/60 border border-slate-200/80 text-[10.5px] text-slate-800 flex items-start gap-1.5 cursor-pointer transition-all shadow-2xs"
              title="Click to view full remark notes"
            >
              <MessageSquare className="w-3 h-3 text-indigo-600 shrink-0 mt-0.5" />
              <div className="flex-1 min-w-0">
                <span className="font-bold text-indigo-700 mr-1">
                  {task.remarks?.[0]?.author_name ? `${task.remarks[0].author_name}:` : 'Note:'}
                </span>
                <span className="text-slate-600 italic truncate inline-block max-w-full">
                  "{task.latest_remark || task.remarks?.[0]?.text}"
                </span>
              </div>
              {task.remarks?.length > 1 && (
                <span className="px-1.5 py-0.2 rounded-full bg-indigo-100 text-[9px] font-bold text-indigo-800 shrink-0">
                  +{task.remarks.length - 1}
                </span>
              )}
            </div>
                ...
                return (
                  <div key={b.id || i} className="p-2 rounded-lg bg-white border border-slate-200/90 shadow-2xs space-y-1">
                    <div className="flex items-center justify-between gap-1 text-[11px]">
                      ...
                      <span className={`px-1.5 py-0.2 rounded text-[9px] font-extrabold shrink-0 ${isDone ? 'bg-emerald-100 text-emerald-800' : 'bg-blue-100 text-blue-800'
                        }`}>
      ...
      {/* Footer Row */}
      <div className="mt-auto pt-2.5 border-t border-slate-100 flex items-center justify-between gap-2 h-7">

        {/* Assignee */}
        <div className="flex items-center gap-1.5 min-w-0">
          <div
            className="w-5 h-5 min-w-[20px] rounded-full flex items-center justify-center text-[9px] font-extrabold text-white shadow-2xs shrink-0 ring-1 ring-slate-200/80"
            style={{ backgroundColor: task.assignee_color || '#4f46e5' }}
          >
            {task.assignee_avatar || '??'}
          </div>
          <span className="text-[11px] text-slate-700 font-medium truncate max-w-[110px]" title={task.assignee_name}>
            {task.assignee_name}
          </span>
        </div>
        ...
        <div className="relative inline-block">
          <select
            ...
            className={`h-6 text-[10px] font-bold pl-2 pr-5 py-0.5 rounded-lg border appearance-none cursor-pointer focus:outline-none focus:ring-1 transition-all ${task.status === 'completed'
                ? 'bg-emerald-50 text-emerald-800 border-emerald-200/80 focus:ring-emerald-400'
                : task.status === 'in_progress'
                  ? 'bg-indigo-50 text-indigo-800 border-indigo-200/80 focus:ring-indigo-400'
                  : task.status === 'blocked'
                    ? 'bg-rose-50 text-rose-800 border-rose-200/80 focus:ring-rose-400'
                    : 'bg-slate-100/90 text-slate-700 border-slate-200/80 hover:bg-slate-200/80 focus:ring-slate-400'
              }`}
          >
            {task.is_book_reading ? (
              <>
