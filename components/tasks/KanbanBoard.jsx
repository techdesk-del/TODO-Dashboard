...
export default function KanbanBoard({ 
  ...
  const [mobileMemberTab, setMobileMemberTab] = useState({});
  ...
  return (
    <div className="space-y-4 w-full">
      ...
      {filteredTasks.length === 0 ? (
        ...
        <div className="space-y-4 w-full">
          {/* 1. Mobile Cards View (EXCLUSIVELY on < lg screens: Phones & Tablets) */}
          <div className="lg:hidden space-y-4">
            {memberMatrixData.map((member) => {
                const user = member.user;
                const activeTab = mobileMemberTab[user.id] || 'in_progress';
                const setTab = (t) => setMobileMemberTab(prev => ({ ...prev, [user.id]: t }));
                ...
                let visibleTasks = [];
                if (activeTab === 'in_progress') visibleTasks = member.inProgressTasks;
                else if (activeTab === 'todo') visibleTasks = member.todoTasks;
                else if (activeTab === 'blocked') visibleTasks = member.blockedTasks;
                else if (activeTab === 'completed') visibleTasks = member.regularCompletedTasks;
                else if (activeTab === 'all') visibleTasks = [...member.inProgressTasks, ...member.todoTasks, ...member.blockedTasks, ...member.regularCompletedTasks];

                return (
                  <div key={user.id} className="bg-white rounded-2xl border border-slate-200/90 shadow-sm p-3.5 space-y-3">
                    {/* Member Header */}
                    <div className="flex items-center justify-between gap-2 pb-2.5 border-b border-slate-100">
                      <div className="flex items-center gap-2.5 min-w-0">
                        <div className="relative shrink-0">
                          <div
                            className="w-9 h-9 rounded-xl flex items-center justify-center text-xs font-bold text-white shadow-xs"
                        ...
                        <div className="min-w-0">
                          <div className="font-bold text-slate-900 text-sm truncate flex items-center gap-1.5">
                            ...
                            <span className="text-[10px] font-medium text-slate-500 bg-slate-100 px-1.5 py-0.2 rounded capitalize shrink-0">
                              {user.role}
                            </span>
                          </div>
                          <div className="flex items-center gap-2 text-[10.5px] text-slate-500 mt-0.5">
                            <span className="font-semibold text-slate-700">{member.total} tasks</span>
                            <span>•</span>
                            <span className="text-emerald-600 font-semibold">{completionRate}% Delivered</span>
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="flex items-center gap-1.5 overflow-x-auto pb-1 no-scrollbar text-xs">
                      {[
                        { id: 'in_progress', label: 'In Progress', count: member.inProgressTasks.length, dot: 'bg-blue-500' },
                        { id: 'todo', label: 'To Do', count: member.todoTasks.length, dot: 'bg-slate-400' },
                        { id: 'blocked', label: 'Blocked', count: member.blockedTasks.length, dot: 'bg-rose-500' },
                        { id: 'completed', label: 'Done', count: completedCount, dot: 'bg-emerald-500' },
                        { id: 'workload', label: 'Workload', count: `${completionRate}%`, dot: 'bg-indigo-500' },
                        { id: 'all', label: 'All', count: member.total, dot: 'bg-slate-600' }
                      ].map(tab => {
                        ...
                        return (
                          <button
                            key={tab.id}
                            type="button"
                            onClick={() => { sounds.playClick(); setTab(tab.id); }}
                    ...
                    {activeTab === 'workload' ? (
                      <div className="p-3 bg-slate-50/80 rounded-xl border border-slate-200/80 space-y-3">
                        <div className="flex items-center justify-between">
                          <span className="text-xs font-bold text-slate-600">Sprint Velocity</span>
                        ...
                        <div className="grid grid-cols-2 gap-2 text-xs">
                          <div className="p-2 bg-white rounded-lg border border-slate-200/70 text-center">
                            <span className="text-slate-500 text-[10px] block">Delivered Tasks</span>
                            <span className="text-sm font-black text-slate-900">{completedCount} of {member.total}</span>
                          </div>
                          <div className="p-2 bg-white rounded-lg border border-slate-200/70 text-center">
                            <span className="text-slate-500 text-[10px] block">Pending Tasks</span>
                            <span className="text-sm font-black text-amber-600">{member.total - completedCount}</span>
                          </div>
                        </div>
                        {member.totalPages > 0 && (
                          <div className="p-2 bg-indigo-50/80 rounded-lg border border-indigo-100 text-xs flex items-center justify-between text-indigo-900">
                            <span className="font-bold flex items-center gap-1.5">
                              <BookOpen className="w-3.5 h-3.5 text-indigo-600" />
                              <span>Reading Goal</span>
                            </span>
                            <span className="font-extrabold">{member.pagesRead} / {member.totalPages} pgs</span>
                          </div>
                        )}
                      </div>
                    ) : visibleTasks.length === 0 ? (
                      <div className="p-6 text-center bg-slate-50/60 rounded-xl border border-dashed border-slate-200 text-slate-400 text-xs">
                        No tasks in this category
                      </div>
                    ) : (
                      <div className="space-y-2">
                        {visibleTasks.map((t, tIdx) => {
                          ...
                          return (
                            <div
                              key={t.id}
                              className={`p-3 rounded-xl bg-white border shadow-2xs space-y-2 transition-all ${
                                isBlocked ? 'border-rose-300 border-l-4 border-l-rose-500' :
                                isCompleted ? 'border-emerald-200 border-l-4 border-l-emerald-500 bg-emerald-50/20' :
                                isTodo ? 'border-slate-200 border-l-4 border-l-slate-400' :
                                'border-blue-200 border-l-4 border-l-blue-500'
                              }`}
                            >
                              {/* Title & Priority Row */}
                              <div className="flex items-start justify-between gap-2">
                                <span className="font-bold text-slate-900 text-xs leading-snug break-words flex-1">
                                  <span className="text-slate-400 font-semibold mr-1.5">#{tIdx + 1}</span>
                                  <span>{t.title}</span>
                                </span>
                                <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded uppercase shrink-0 ${
                                  t.priority === 'urgent' ? 'bg-rose-50 text-rose-700 ring-1 ring-rose-200' :
                                  t.priority === 'high' ? 'bg-amber-50 text-amber-700 ring-1 ring-amber-200' :
                                  t.priority === 'medium' ? 'bg-blue-50 text-blue-700 ring-1 ring-blue-200' : 
                              ...
                              {/* Description */}
                              {t.description && (
                                <p className="text-[11px] text-slate-600 break-words leading-relaxed pl-4 border-l border-slate-200">
                                  {t.description}
                                </p>
                              )}
                              ...
                              {(t.latest_remark || t.remarks?.[0]?.text) && (
                                <div
                                  ...
                                  className="p-1.5 rounded-lg bg-indigo-50/70 border border-indigo-100 text-[10.5px] text-indigo-950 flex items-center gap-1.5 cursor-pointer hover:bg-indigo-100/70 transition-colors"
                                >
                                  ...
                                  {t.remarks?.length > 1 && (
                                    <span className="px-1.5 rounded bg-indigo-200 text-[8.5px] font-bold text-indigo-900 shrink-0">
                                      +{t.remarks.length - 1}
                                    </span>
                                  )}
                                </div>
                              ...
                              {/* Dates & Actions Row */}
                              <div className="flex items-center justify-between gap-2 pt-1.5 border-t border-slate-100 text-xs flex-wrap">
                                <div className="flex items-center gap-2 text-slate-500 text-[10.5px]">
                                  {(t.start_date || t.due_date) ? (
                                    <span className="flex items-center gap-1">
                                      <Calendar className="w-3 h-3 text-slate-400 shrink-0" />
                                      <span>{formatDateRange(t.start_date, t.due_date)}</span>
                                    </span>
                                  ) : (
                                    <span className="text-slate-400">No dates</span>
                                  )}
                                  {t.due_date && new Date(t.due_date).getTime() < new Date(todayStr).getTime() && !isDone && (
                                    <span className="text-[8.5px] font-bold px-1.5 py-0.2 rounded bg-rose-100 text-rose-800 uppercase">
                                      Overdue
                                    </span>
                                  )}
                                </div>
                                ...
                                <div className="flex items-center gap-1.5 ml-auto">
                                  ...
                                  <button
                                    type="button"
                                    onClick={() => { sounds.playClick(); setActiveRemarkTask(t); setActiveRemarkCandidateTasks(candidateTasks); }}
                                    className="p-1 px-1.5 rounded-md hover:bg-slate-100 text-slate-500 hover:text-indigo-600 text-[10.5px] flex items-center gap-1 cursor-pointer transition-colors"
                                    title="Remarks"
                                  >
                                    <MessageSquare className="w-3 h-3 text-indigo-500" />
                                    <span>{t.remarks?.length || 'Remark'}</span>
                                  </button>
                                  ...
                                  <button
                                    ...
                                    className="p-1 rounded-md hover:bg-slate-100 text-slate-400 hover:text-blue-600 cursor-pointer transition-colors"
                                    title="Edit"
                                  >
                                  ...
                                  <button
                                    type="button"
                                    onClick={() => { sounds.playTrash?.() || sounds.playClick(); onDeleteTask(t.id); }}
                                    className="p-1 rounded-md hover:bg-rose-50 text-slate-400 hover:text-rose-600 cursor-pointer transition-colors"
                                    title="Delete"
                                  >
                                  ...
                                  {isTodo && (
                                    <button
                                      ...
                                      className="py-1 px-2.5 rounded-md bg-blue-600 hover:bg-blue-700 active:scale-95 text-white font-bold text-[10.5px] flex items-center gap-1 shadow-xs cursor-pointer"
                                    >
                                      <span>Start →</span>
                                    </button>
                                  )}
                                  {t.status === 'in_progress' && (
                                    <button
                                      type="button"
                                      onClick={() => { sounds.playClick(); onEditTask(t); }}
                                      className="py-1 px-2 rounded-md bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 font-bold text-[10.5px] flex items-center gap-1 cursor-pointer"
                                    >
                                  ...
                                  {isBlocked && (
                                    <button
                                      type="button"
                                      onClick={() => { sounds.playClick(); onStatusChange(t.id, 'in_progress'); }}
                                      className="py-1 px-2 rounded-md bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-[10.5px] flex items-center gap-1 shadow-xs cursor-pointer"
                                    >
                                      <span>Unblock →</span>
                                    </button>
                                  )}
                                  {isDone && (
                                    <button
                                      type="button"
                                      onClick={() => { sounds.playClick(); onStatusChange(t.id, 'in_progress'); }}
                                      className="py-1 px-2 rounded-md bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-[10.5px] flex items-center gap-1 cursor-pointer"
                                    >
                                      <span>Reopen ↺</span>
                                    </button>
                                  )}
          ...
          <div className="hidden lg:block bg-white rounded-2xl border border-slate-300 shadow-sm w-full lg:overflow-visible">
            ...
            <thead>
              <tr className="bg-slate-900 text-white font-bold text-[10.5px]">
                ...
                <th style={{ width: '17%' }} className="sticky top-14 z-30 py-2.5 px-2 border-b-2 border-slate-800 border-b-emerald-500 bg-slate-900 rounded-tr-2xl shadow-xs">
